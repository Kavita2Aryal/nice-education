...
abstract class ModelService
{
	...
	public function searching($columns, $keyword)
	{
		$query =  $this->query();
	...
	public function changeOrder(array $orderedCategoryIds)
	{
		foreach ($orderedCategoryIds as $key => $orderedCategoryId) {
			$model = $this->findOrFail($orderedCategoryId);
			$model->update(['order' => $key + 1]);
		}
	}

	public function setDefaultImage($column, $id)
	{
		$model = $this->findOrFail($id);
		return $model->update([$column => null]);
	}
}
